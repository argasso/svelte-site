import { asArray, unSlug } from '$lib/utils/util'
...
export function createCategoryParameters(categories: string[]): FilterParameter[] {
  return categories
    .sort()
...
export function getMostSpecificCriteria(
  ...
  const queryCriterias = asArray(query.get(key))
  const criterias = additionalValue ? [...queryCriterias, additionalValue] : queryCriterias
