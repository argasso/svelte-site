 ...
 *
 * @param {SvelteComponent} Component SvelteComponent import from .svelte
...
export default function SvelteAdapter(
      ...
      if (!instance) {
        ...
        for (const key in props) {
          ...
          if (eventMatch && typeof props[key] === 'function') {
            instance.$on(`${eventMatch[1][0].toLowerCase()}${eventMatch[1].slice(1)}`, props[key])
          }
  ...
  Wrapper.displayName = displayName ? displayName : `SvelteAdapter/${Component.name}`
